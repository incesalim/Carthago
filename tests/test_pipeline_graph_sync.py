...
from check_pipeline_graph_sync import (
    SCRATCH_WORKFLOWS,
    check,
    dead_hrefs,
    routes,
...
def test_scratch_exemptions_name_a_workflow_that_still_exists():
    """A scratch lane is exempt because it draws no lineage, not forever. When
    the lane is deleted the exemption must go with it, or the next scratch
    workflow reusing the name inherits a silent pass."""
...
def test_scratch_exemptions_carry_a_reason():
    for name, reason in SCRATCH_WORKFLOWS.items():
