...
-- Sourced from the same "Information by major sectors" footnote as
-- bank_audit_loans_by_sector, but the TL/FC columns rather than Stage 2/3/ECL.
-- Additive: does not touch the settled loans_by_sector stage columns.
CREATE TABLE IF NOT EXISTS bank_audit_loans_currency (
    ...
    tl_pct           REAL,               -- percentage of TL (nullable)
...
-- Sectors" table in the Pillar 3 footnote section.
