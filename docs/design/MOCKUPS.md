...
|---|---|---|---|
| **Control center** (`/admin`) — the pipeline-health / triggers / traffic console redesigned into a Desk page | **SHIPPED 2026-07-15** — built deploy-ready into the live code: `web/app/admin/page.tsx` (Desk shell), `PurgeCacheButton`/`PipelinePanel`/`TrafficPanel`/`coverage/CoverageMatrix` all restyled to hairlines + mono-caps controls, **every** byte of function preserved (six data-health sources, coverage matrix + per-cell re-extract, all pipeline triggers, traffic, the PDF deck, purge-cache, password gate, sign-out). tsc + eslint + 297 tests + webpack build all green. **The one departure from this mockup: the top computed-status "Read" sentence was dropped at the user's request** — the page opens straight into the vitals band. Premise it fixed: `/admin` was the last surface never converted to the Desk — boxed `Stat` tiles, pill freshness badges, and the framing "freshness per source, **against expected refresh cadence**", the exact age-vs-cadence logic behind the old daily false "stale" alarm (already fixed in code: freshness asks BDDK if the next month is published). What shipped: the six sources are the **vitals band** (the one bold element), each cell a period figure + status dot + one ground-truth line (monthly prints "June not yet published · probed today", never "updated Nd ago"); the DeskHeader record line computes "all 6 sources current" vs a flagged list; Fresh/Late/Stale stay green/amber/red as **semantic state** (separate from the blue link accent); controls are mono-caps ink-underline affordances; a `Colophon` closes the page. | [html](mockups/2026-07-15-admin-page.html) | [artifact](https://claude.ai/code/artifact/f439323e-3a7f-4ea9-a1a5-ab773bdc0003) |
