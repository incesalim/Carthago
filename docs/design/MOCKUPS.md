...
- **Regulation implies extraction work, not just UI.** The Rulebook mockup needs three
  ...
  **`is_instrument`** rule so an SSL certificate and a magazine stop counting as
  ...
  decisions should also be tagged to tickers with `bank_tagger.py`: **FUPS Bank holds a
  BDDK operating licence and is absent from `banks`**, which is a coverage gap the page
  could have raised by itself.
