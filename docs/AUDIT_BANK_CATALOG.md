...
| ISCTR | **KAP filing and IR PDF are separate events (2026-08-16).** İş Bankası filed 2026Q2 on KAP on 08-03; two weeks later its own financial-statements page still offered only `ISBTRUnconsolidatedFinancials30062026.xlsx` — the Excel, no PDF, for either basis (verified in a browser; the page is an ASP.NET postback list that no HTTP probe can enumerate). This is the shape `filing-season.ts` calls `results_only`, and it is normal for this bank rather than a fault. Nothing to acquire until the PDF appears. |
