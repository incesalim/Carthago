...
|---|---|---|
| **group** | profitability · income · balance_sheet · growth · asset_quality · capital · liquidity_funding · franchise · market_position · esg · macro | Metric family. |
...
Current snapshot (v1 seed, 35 metrics): **20 / 35** reproducible from audit
reports; the 8 we can't get at all are the swap-adjusted NIM, CPI-linker income,
LCR, wholesale-funding detail, restructured-loan ratio, subsidiary AuM, ESG
ratings, and per-bank active-digital-customers.

## How it was seeded & how to extend it

The v1 seed was built by classifying the metrics in the Akbank 1Q26 consolidated
earnings presentation against our holdings (several entries carry a verified
`examples` cross-check — e.g. ROE 25.3%, Stage 2+3 11.4%, both matching our
`compute_bank_metrics` output for AKBNK 2026Q1 consolidated).

To extend: add an entry to `registry.json` conforming to `schema.json`, run
`python scripts/metric_knowledge.py --validate`, and (if it maps to a chart we've
reproduced) link its `spec_ids`. Keep the helper's `ENUMS` in sync with the
schema — `tests/test_metric_knowledge.py::test_enum_parity_with_schema` enforces it.
