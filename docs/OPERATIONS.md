...
| Fri 13:30 + 15:30 UTC, Sat 02:00 UTC | `refresh-bddk-bulletins.yml` | BDDK bulletins (no EVDS, no audit) → D1. The two **Friday** runs (16:30 & 18:30 Turkey) are **weekly-only** — BDDK publishes the weekly bulletin Friday afternoon (~16:00–18:00 Turkey), so we grab it the same evening; the **Saturday** run does weekly + monthly. Sends a positive Telegram ping ("published & fetched") when a new weekly/monthly period lands (`notify_new_bddk.py`); quiet otherwise |
