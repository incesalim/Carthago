...
| Manual only | `repair-missing-audit-rows.yml` | Restore missing D1 rows from the latest authoritative R2 audit snapshot without extraction or re-stamping. Inputs: explicit allowlisted `tables` (no ALL), optional `banks`/`periods`, `kind=both`, `dry_run=true`. Sequential grouped counts find whole/partial partition losses; complete factual multisets must be strict source subsets (null is distinct from zero). Missing tables, schema drift, conflicting/extra facts, or incomplete reads abort **all selected tables before any write**. Equal-count partitions are not value-audited or replaced. Actions-only apply uses exact per-table `--replace-partitions`, verifies all restored facts and a no-op second comparison, then uploads updated snapshot digests. Unchanged runs write neither D1 nor R2. Uses existing R2 and `CLOUDFLARE_API_TOKEN` secrets; serialized with `bddk-audit` |
