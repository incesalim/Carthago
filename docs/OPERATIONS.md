...
Explicit corpus dispatches keep their own `publish` input. Recovery still
selects only published source revisions from capture artifacts and skips
cancelled/skipped parents.
...
| Daily 06:00 UTC | `healthcheck.yml` | D1 freshness check + **filing-season gap** (`filing_gap_problem`) + **coverage-spine check** (`audit_spine_problem` — alerts when the `audit_spine` sync marker is missing, D1 holds fewer lanes than the registry declares, or the spine is >10d frozen) + **coverage-trend snapshot** (`write_coverage_trend` — one `coverage_trend` row per run, migration 0055, read by /admin for error/missing deltas) + `verify_chart_spec.py` + `check_amount_integrity.py` + Telegram webhook-target check → Telegram/Discord alert if stale/failing/drifted/unacquired |
...
| `OPEN_ROUTER_API` | OpenRouter (DeepSeek et al). Added 2026-07-05; **since 2026-08-17 it is the head of every lane's chain** — `generate-reads.yml`, `summarize-regulations.yml` (when `BRIEFING_LLM=deepseek-flash`), the analyst workflows. ⚠️ Note the name: no `_KEY` suffix, unlike every other provider secret. ⚠️ This is the **Actions** secret; the Worker needs its own copy via `wrangler secret put` |
...
- **A new lane is registered but absent from the matrix** — the matrix renders
  what the spine synced, so a lane added to the registry is INVISIBLE until the
  next `refresh-audit` push inserts its `bank_audit_statement_types` row. The
  vitals strip under the Coverage head now says so instead: "spine synced Xh
  ago · 19/20 lanes in D1" goes amber when D1 lags the registry. `push_to_d1`
  stamps `source_freshness ('audit_spine', latest_period = lane count)` on
  every push that ships the spine tables — that stamp is the strip's data
  source, and `healthcheck.py` (`audit_spine_problem`) alerts on a missing
  marker, a lane-parity gap, or a >10-day-frozen spine.
