...
- **Only figures ≥1000 were checked**, so every ratio was unverified — the most-asked question shape, and three of the four documented escapes (`%16,2`, `NPL is 2.3%`, `ROE was 38,5%`). **Fixed 2026-08-03:** anything written as a percentage is now checked at any magnitude. A percentage is never a rank or a bank count, so it carries none of the noise the floor exists to prevent. It is matched at **×1 and ×100**, because the corpus stores ratios both ways — `npl_coverage` is a fraction (0.0083 *is* 0.83%) while `cet1`/`car`/`lcr` are already points (17.06 *is* 17.06%) — with a tolerance of half a unit in the last printed place, so a correctly-rounded figure still matches the value it was rounded from.
- ⚠️ **The guard was Turkish-only, and the bot is not.** It answers in the language of the question, so an English reply writes `2.3%` and `51,760,765` — exact mirrors of the Turkish convention. `numbersInProse` assumed Turkish and read `2.3%` as **23**, an order of magnitude out, on every English answer since the lane shipped. Fixed in the same change: `splitProseNumber` needs no language hint — a thousands separator always leaves groups of exactly three digits behind it and a decimal separator almost never does; where both characters appear the last one is the decimal point. The single genuinely ambiguous form, one separator with exactly three digits after it (`1.234`), keeps its Turkish reading.
...
covered by the gates (`bot-sql.test.ts` + `bot-failure.test.ts`, 95 tests) and by `bot_queries` after the
