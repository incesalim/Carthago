...
| `npl_movement` | 641 | **0** | 334 | **2026-06-21: 126→0** (FX "Kur farkı" row + closing-vs-`npl_brsa_gross` cross-check skip-if-bottom-line-right + HALKB total-block extractor fix + PASHA outflow-magnitude `abs()`) |
| `loans_by_sector` | 150 | **21** | 804 | **annual-only** disclosure (interim has no table). **2026-06-21: 36→21** — YKBNK (22) was extracting the WRONG table (capital/equity rows): locator missed "Information ACCORDING TO sectors and counterparties" + false-matched the risk-profile & investments tables; fixed locator + added YKBNK sector wordings (re-extracted, annual-cons cleared). **Remaining 21** = per-bank multi-column table structures: QNBFB 8 (5-col Stage2\|Stage3\|Prov\|ECL\|Prov — the "3 trailing numbers" heuristic grabs the dash/ECL cols), YKBNK 7 (3 interim-unco stale wrong-table rows the re-extract didn't clear + 4 annual-unco svc-row parse gaps), EXIM/ICBCT 2 each, BURGAN/KLNMA 1. Needs x-coordinate column alignment (the lane parses text lines, not word positions) |
