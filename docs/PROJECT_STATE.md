...
| `bank_audit_free_provision` | BRSA PDFs, "Other provisions" liability note | 2022-Q1 → 2026-Q1 | **the free provision (serbest karşılık)** — the discretionary reserve behind the ALBRK case, held nowhere before. `free_provision` current STOCK (0 = explicit none) + prior + `source_text`. Deterministic text classifier (`src/audit_reports/free_provision.py`), EN+TR, 3 disclosure forms, flow-vs-stock veto (incl. TR "iptal edilmiştir" = cancelled, not stock; prefers the auditor's stated TOTAL over a note sub-line); 11/11 on a hand-verified sample. **Backfilled 2026-07-15**: 501 rows / 26 banks, **437 holding, 64 none**. Guarded by the `freeprov` validator in `check_audit_quality.py` (band + longitudinal prior-chain + two-sided opinion cross-check). Hand-transcribed overrides in **`data/free_provision_overrides.json`** (77 partitions, read from the full auditor qualifications — composite "X gecmis − Y iptal = toplam Z", counterfactual "diger karsiliklar X daha az", decomposition "400k+1,650k") win over extraction; source_page=−1. free_provision=0 = held-then-fully-cancelled (ZIRAATK 2024, ZIRAAT 2025Q4). Validator progress: **recall gaps 80→25, total anomalies 86→36**. 556 rows / 485 holding / 71 explicit-zero. **Residual:** ~25 recall + ~9 prior_chain (FIBA consol, VAKBN 2024Q4/2025Q4, TEB-2025, BURGAN-2026, ALNTF, EMLAK-chain) — the harder multi-figure notes, honestly left as tracked gaps, NOT fabricated. NOTE: an earlier EMLAK override was WRONG (used 855,130 = the deferred TAX on the provision); corrected to 2,850,432/5m/7.5m — cross-check any override against the auditor's own figure, not an adjacent number. Re-extract is delete-then-insert (authoritative). Backfill: `reextract-statement.yml` statement=`free_provision`, force, only_failing off |
