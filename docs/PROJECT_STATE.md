...
| `stages` | 1015 | 10 | 25 | **2026-07-16:** carried by the `credit_quality` floor fix above — coverage missing 14→5, ok 1021→1024, n/a 8→11. The 10 fails are one class, `stages_stage3_missing` (S1/S2 captured, S3 NULL): new digital banks that print §7.2 but no III/IV/V NPL table at all (COLENDI 2025Q4, TOMK 2024Q2–Q4, ZIRAATD 2025Q4, DUNYAK 2025Q1/Q2). 3 of these are newly VISIBLE rather than newly broken — they were 'missing' before the S1/S2 rows existed. Left flagged deliberately: the check's premise ("a zero-NPL bank stores S3=0, not NULL") assumes the bank prints a zeroed NPL table, which these don't — but defaulting S3 to 0 on an absent table would rebuild exactly the blind spot that hid EMLAK's missing NPL for 10 quarters. Deciding 0-vs-N/A for "no NPL table filed" is an open call. NPL=100% **fixed end-to-end 2026-06-15**; residual 15 cleared 2026-06-21 (credit_quality fitz migration + per-bank `loans_by_stage` cluster fixes). (1) Validator: the NPL=100% fingerprint required stage1/stage2 non-null but the broken shape has them NULL → it skipped all 45, which showed green; now NULL counts as 0 → 45 surfaced. (2) Extractor (`credit_quality.loans_by_stage`): captured the §7.2 Stage-1/2 table on 3 column-split variants (İşbank EN/no-space coord fallback; ANADOLU wrapped header → Stage-2-only anchor; TSKB label/number y-offset → 5.5px cluster). Re-extracted 6 banks → rebuilt derived stages → **43 of 45 repaired** (npl100 45→2). Remaining 2 = FIBA + TFKB image-only quarters |
