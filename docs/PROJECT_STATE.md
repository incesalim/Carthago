...
| `bank_audit_tl_fc_note_full` | Minted from the DOCUMENT layer by `scripts/build_tl_fc_note_full.py` — the eighteenth graduated lane, five P&L / BS breakdown notes | **built 2026-08-21: 627 filings / 1,132 instances / 5,452 rows (158 instances refused), local `data/bank_audit_tables.db`, not D1** | Items × (TL, FC) × (current, prior): interest on loans 274 instances, interest from banks 237, interest on securities 68, interest on funds borrowed 183 (nested under "to banks"), funds borrowed 370. Gate: total = Σ rows and head = Σ children. Cross-lane, current TL+FC of the total vs the narrow statement line: loans 100% of 258, funds borrowed 100% of 354, securities 98.3%, borrowings 93.4%, from banks 83.7% (the narrow P&L lacks the line in some filings — AKBNK 2022Q4 has no "Bankalardan alınan faizler" row at all, a narrow gap). Roles on 98.8% of value-bearing rows |
