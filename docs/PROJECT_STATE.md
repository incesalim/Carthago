...
- `.github/workflows/refresh-bddk-bulletins.yml` — Fri 13:30 + 15:30 UTC (weekly-only) + Sat 02:00 UTC (weekly + monthly). BDDK bulletins (no EVDS, no audit) → D1. The Friday runs (16:30 & 18:30 Turkey) grab the weekly bulletin the same afternoon BDDK publishes it (~16:00–18:00 Turkey) instead of waiting for Saturday; scheduled Friday runs skip monthly via `github.event.schedule` (the monthly bulletin lands mid-month). Idempotent, so the Saturday backstop is harmless overlap.
