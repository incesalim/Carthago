...
Status: active. Complete the registered corpus before adding banks/history.
Retain prior document experiments as evidence within the existing pipeline.

## Current execution order (2026-09-13, supersedes priorities below)

The user explicitly requires the following sequence. Equity/capital were examples;
the scope is every entry shown in the admin matrix. Do not substitute another
small selected-field expansion or prose-reader change for completing this order.

1. Independently audit every extractor, retained schema, loader, actual candidate
   gate and curated skip. Compare source PDFs; remeasure gate weaknesses instead
   of reusing old blind-spot percentages. Record source omissions separately from
   numerical interpretation and arithmetic contradictions.
2. Capture each listed source table fully and integrate its dipnotes. Preserve
   native rows/columns (including nonstandard ones), headings, all printed periods
   and currencies, units, signs, raw cells, blanks/dashes/zeros, page/geometry and
   source hash. Associate table-, row- and column-level note markers with their
   full explanations; retain cross-section addresses and unresolved references.
   Derived `stages` is lineage to credit quality, not a second source table.
3. Repair validators against that source contract. Test missing rows/columns,
   omitted notes, wrong labels, periods, signs and units as well as arithmetic.
   A correctly transcribed source contradiction remains visible; it is not fixed
   by changing a number or removing the publication guard. A source manifest
   generated by the same parser cannot independently certify that parser.
4. Organize prose by the filing's actual sections, headings, note addresses and
   passages; link notes back to tables and expose meaningful coverage by section
   and content role. A single "prose" count is not a completion measure.

Release through exact-partition Actions trials and scoped publication, preserving
settled and manually corrected data. Keep the analytical series as projections of
the full source tables; adding a native source representation does not justify
overwriting every stored financial row. Completion requires corpus-wide evidence,
not a pilot, row minimum, green tick or successful workflow.

### Audit instrumentation and verified initial gaps

`audit-extraction-gates.yml` runs a read-only diagnostic on one ETag-pinned audit
snapshot. `scripts/diagnostics/audit_extraction_gates.py` recomputes every selected
registered filing/lane through the actual `revalidate_partition` and candidate
gate, then injects table/row/column omissions, cell changes/NULLs/sign flips, erased
labels/notes/evidence, current/prior swaps and coherent 1,000x filing-wide monetary
scaling. Every fault rolls back. Initially
failing, empty or unproven lanes are excluded from escape-rate denominators.
Default sampling is two faults per operator per eligible filing/lane; reports
include the number of available candidates. This is not a PDF-accuracy test and
does not certify table completeness. The snapshot itself is opened read-only;
the audit mutates only an in-memory backup and verifies the input file hash.

Fresh source/code review identifies these contract gaps before expansion:

- Primary `StatementRow` objects carry current and comparative amounts, but the
  balance/off-balance serving tables retain only the current currency triplet;
  P&L/OCI/cash-flow serving tables retain only the current amount. A different
  historical filing cannot replace the comparative printed here after restatement.
- TOMK 2023Q3 assets, physical PDF page 10: the dedicated Dipnot (V-I) column
  links Bankalar to (1), and intangible assets to (4). The current parser returns
  those markers inside `item_name` while `footnote` is NULL. Roman/section references
  also have masking paths that need source-specific review before repair.
- `document_table_notes.py` links only a consecutive numbered-column list below
  a candidate table on the same page. It is not a general table/row/cross-section
  note resolver and is not integrated with the analytical table rows.
- Credit quality, capital, liquidity, FX and repricing explicitly retain selected
  facts. Their source-capture validator permits unmapped rows under
  `selected_summary`; a passing result therefore cannot mean full table capture.
- The matrix tick denotes that a validator exists. Its prose row counts legacy
  D1 prose, whereas the structured admin reader uses R2. Neither signal proves
  semantic structure or all-source coverage.
- `loader.upsert_report` catches every revalidation exception and continues to
  write the extraction log; its success field is based on core row counts. The
  new audit reports exceptions separately. This loader failure path also needs
  repair; it cannot be counted as successful validation.
...
## Earlier prose delivery priority (2026-09-08, superseded)
...
|---|---|---|
| balance_sheet_assets | Parser has six current/prior cells; serving rows retain only current TL/FC/total | Restore native comparative periods and reconcile every printed row/header/dipnot; keep settled values protected |
| balance_sheet_liabilities | Parser has six currency cells; serving rows retain only the current triplet | Same, including displaced amounts and their label association |
...
| off_balance | Parser has six currency cells; serving rows retain only the current triplet | Preserve every source period/column, row label, note and raw zero/dash/blank |
