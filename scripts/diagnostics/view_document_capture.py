...
Examples
--------
  python scripts/diagnostics/view_document_capture.py --bank AKBNK --period 2026Q1
  python scripts/diagnostics/view_document_capture.py --bank AKBNK --period 2026Q1 --pages 41,46
  python scripts/diagnostics/view_document_capture.py --list
"""
...
# Section structure (the contents model, folio placement, banners) is shared
# with scripts/graduations/build_document_tables.py via src/audit_reports/document_sections
# — one implementation, so the viewer's banners and the queryable lane's
# section columns cannot drift apart.
from src.audit_reports.document_sections import (  # noqa: E402
    RALT as _RALT, ROMAN as _ROMAN, document_contents as _document_sections)
