...
| `build_tl_fc_note_full.py` | The TL/FC notes graduation — the small notes printed as items × (TL, FC) × (current, prior), five families under a registry: interest on loans, interest from banks, interest on securities, interest on funds borrowed (nested: "to banks" heads four sub-rows), funds borrowed. Gate: total = Σ top-level rows and head = Σ children on the current TL and FC columns. Anchor: current TL+FC of the total vs the family's narrow P&L / BS line. The balance-sheet "banks" note wears the interest-from-banks rows and is left unminted (told apart by the contents item). → `bank_audit_tl_fc_note_full` in `data/bank_audit_tables.db` (local, not D1). | by hand | operational |
