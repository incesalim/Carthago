...
- **`Segmented`** — the single segmented-control idiom: bordered pill container, active segment always `bg-primary/10 text-primary font-semibold`. Use for new toggles; bespoke toggles that can't adopt the markup must still use these active classes (RangePills, HeatmapView tabs, PlSankey period pills, NimComponents toggles all conform).
