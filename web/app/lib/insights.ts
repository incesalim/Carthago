...
/** Loans by sector — "where is the credit, and where is the risk concentrated?" */
export function loansBySectorInsights(d: {
  asOf: string | null;
  headlineNpl: number; // sector NPL ratio, %
  bookYoYReal: SeriesPoint[]; // whole-book real (CPI-deflated) growth, %
  consumerShare: SeriesPoint[]; // consumer group share of the book, %
  topGroup: { label: string; share: number } | null; // largest super-group
  worstSector: { label: string; nplRatio: number } | null; // highest-NPL material sector
  bestSector: { label: string; nplRatio: number } | null; // lowest-NPL material sector
}): TabTakeaway {
  const items: Insight[] = [];
  const npl = d.headlineNpl;

  // 1. Where the credit sits — concentration.
  const cShare = last(d.consumerShare);
  if (d.topGroup) {
    items.push({
      text:
        `The book leans on ${d.topGroup.label.toLowerCase()} — ${pct(d.topGroup.share)} of loans` +
        (cShare != null ? `, with consumer credit ${pct(cShare)} of the total` : "") +
        ".",
      tone: "neutral",
    });
  }

  // 2. Where the risk sits — the dispersion the calm headline hides.
  if (d.worstSector && d.bestSector) {
    items.push({
      text:
        `The ${pct(npl, 1)} headline NPL hides its spread — ${d.worstSector.label.toLowerCase()} runs ` +
        `${pct(d.worstSector.nplRatio, 1)}, against ${d.bestSector.label.toLowerCase()} at ${pct(d.bestSector.nplRatio, 1)}.`,
      tone: d.worstSector.nplRatio > 2 * npl ? "warn" : "neutral",
      href: "/asset-quality",
    });
  }

  // 3. How the mix is shifting — consumer share direction over 12 months.
  const cD = deltaOver(d.consumerShare, 12);
  if (cShare != null && cD != null) {
    const move = direction(cD, VERBS.move, { flat: 0.3, sharp: 1.5 });
    items.push({
      text: `Consumer credit's share of the book ${move ?? "held"} ${ppStr(cD)} over 12 months.`,
      tone: "neutral",
    });
  }

  // 4. Real growth — the book stripped of inflation.
  const real = last(d.bookYoYReal);
  if (real != null) {
    items.push({
      text: `Stripped of inflation, the loan book ${real < 0 ? "shrank" : "grew"} ${pct(Math.abs(real))} over the year.`,
      tone: real < 0 ? "warn" : "neutral",
    });
  }

  const headline =
    d.worstSector && d.topGroup
      ? `The book concentrates in ${d.topGroup.label.toLowerCase()} (${pct(d.topGroup.share)}); the ${pct(npl, 1)} headline NPL ` +
        `masks ${d.worstSector.label.toLowerCase()} at ${pct(d.worstSector.nplRatio, 1)}` +
        (real != null
          ? `, on a book that ${real < 0 ? "shrank" : "grew"} ${pct(Math.abs(real))} in real terms.`
          : ".")
      : `The sectoral loan book carries a ${pct(npl, 1)} headline NPL.`;

  return { asOf: d.asOf, headline, items };
}
