...
export const STATEMENTS: Record<string, StatementSpec> = {
  ...
  npl_movement: {
    ...
    columns: [
      ...
      "collections", "write_offs", "sold", "fx_diff", "accrual_movement", "other_movement", "closing_balance", "provision",
      "net_balance", "source_page",
    ],
    rowIdentity: ["group_code"],
    numericColumns: [
      "opening_balance", "additions", "transfers_in", "transfers_out", "collections",
      "write_offs", "sold", "fx_diff", "accrual_movement", "other_movement", "closing_balance", "provision", "net_balance",
    ],
    hasPeriodType: true,
    hasSourcePage: true,
    caveats: [
      ...
      "other_movement is a separately disclosed signed Other flow, including reclassifications explained by source dipnotes; NULL does not mean zero, and it is not a sale or FX amount",
    ],
  },
  credit_quality: {
  ...
  liquidity: {
    ...
    hasPeriodType: true,
    hasSourcePage: true,
    caveats: ["Selected ratios and eight LCR component amounts, not full liquidity-table coverage. Ratios are percent; components are thousand TL, total includes FC. Cash inflows/outflows are weighted; HQLA and net outflows are capped. These are reported averages: do not equate the reported ratio to a ratio of averages. Evidence retains literal cells, column positions, units, pages and period headings; historical NULL components have not been backfilled.", "nsfr is NULL before 2024Q1 fleet-wide — a disclosure gap, not a zero"],
  },
  fx_position: {
