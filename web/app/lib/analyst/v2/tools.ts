  ...
  if (name === "npl_movement_footing") {
    const rows = await ctx.db.all<Record<string, number | string | null>>(
      "SELECT group_code, opening_balance, additions, transfers_in, transfers_out, collections, " +
        "write_offs, sold, fx_diff, accrual_movement, other_movement, closing_balance FROM bank_audit_npl_movement " +
    ...
    const per = rows.map((r) => {
      ...
      const computed = n("opening_balance") + n("additions") + n("transfers_in") - Math.abs(n("transfers_out")) - Math.abs(n("collections")) - Math.abs(n("write_offs")) - Math.abs(n("sold")) + n("fx_diff") + n("accrual_movement") + n("other_movement");
      return {
        group: r.group_code,
        formula: "opening + additions + transfers_in − |transfers_out| − |collections| − |write_offs| − |sold| + fx_diff + accrual_movement + other_movement",
